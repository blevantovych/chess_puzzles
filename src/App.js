...
const getState = (index = getPositionId(positions)) => {
  return {
    ...
    originalFen: positions[index].fen,
    ...
    return (
      <div className="App">
        <Board
          ...
          makeMove={({ from, to }) => {
            const orientation =
              this.state.position.condition === '1. ?' ? 'w' : 'b'
            const chess = new Chess()
            chess.load(`${this.state.position.fen} ${orientation} - - 1 1`)
            const solutionMoves = this.state.position.solution
              .replace(/\(.+\)/, '')
              .replace(/\$\d+/, '')
              .replace(/\d+\.+/g, '')
              .trim()
            ...
            if (isMoveValid) {
              this.setState({
                position: {
                  ...
                  condition:
                    this.state.position.condition === '1. ?' ? '1... ?' : '1. ?'
                }
              })
            }

            const mCorrect = chess2.move(solutionMoves[this.state.moveCount], {
              sloppy: true
            })

            if (
              chess.fen().replace(/\s.+/, '') ===
              chess2.fen().replace(/\s.+/, '')
            ) {
              if (solutionMoves.length > this.state.moveCount + 1) {
                setTimeout(() => {
                  const isMoveValid = chess2.move(
                    solutionMoves[this.state.moveCount + 1],
                    { sloppy: true }
                  )
                  ...
                  if (isMoveValid) {
                    this.setState({
                      ...
                      position: {
                        ...
                        condition:
                          this.state.position.condition === '1. ?'
                            ? '1... ?'
                            : '1. ?'
                      }
                    })
          ...
          originalFen={this.state.originalFen}
          ...
          prev={() => {
            this.state.index > 0 &&
              this.setState(getState(this.state.index - 1))
          }}
        />
      </div>
    )
